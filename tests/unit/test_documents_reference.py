...
No DB: ``DocumentCarrier`` is a plain dataclass and ``_reference_lines`` a pure
helper over it.
"""
...
from src.modules.optimizations.documents import _reference_lines

_META_STYLE = getSampleStyleSheet()["Normal"]


def _fake_order(notes):
    """Minimal duck-typed order for ``DocumentCarrier.from_order``."""
...
def test_from_payload_without_notes_defaults_to_none():
    carrier = DocumentCarrier.from_payload({}, client=None, reference="PRE-2026-0001")
...
def test_from_order_propagates_the_frozen_reference():
    ...
    carrier = DocumentCarrier.from_order(_fake_order("Obra Los Álamos — cocina"))
    assert carrier.notes == "Obra Los Álamos — cocina"


def test_reference_lines_omitted_without_a_reference():
    for blank in (None, "", "   "):
        carrier = DocumentCarrier(reference="PRE-1", client=None, notes=blank)
